...
GameObject::GameObject(GameSettings& game_settings) {
  if ((game_settings.network.isServer == 0) && (game_settings.network.isNetwork == 1)) {
    ...
    this->platform.frst_platform =
        Platform((game_settings.playing_field_settings.x_playing_field -
                  game_settings.platform_length) /
                     2,
                 game_settings.platform_length);
    this->platform.scnd_platform =
        Platform((game_settings.playing_field_settings.x_playing_field -
                  game_settings.platform_length) /
                     2,
                 game_settings.platform_length);
    this->platform_controllers.frst =
        PlatformControllerFactory::newPlatformController(
            game_settings.ai_settings.AICheck, 'w', 's',
            game_settings.ai_settings.difficulty);
    this->platform_controllers.scnd =
        PlatformControllerFactory::newPlatformController(
